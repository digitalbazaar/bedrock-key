...
const signatureAlgorithm = require('signature-algorithm');
...
api.addPublicKey = (actor, publicKey, privateKey, callback) => {
  ...
  async.auto({
    ...
    insert: ['generateId', (results, callback) => {
      ...
      // FIXME: this is a sticky wicket, there is a unique compound index
      // this uses `pem`, how to make this work in a backward compatible way?

      const pubKey = publicKey.publicKeyPem || publicKey.publicKeyBase58;

      const record = {
        id: database.hash(publicKey.id),
        owner: database.hash(publicKey.owner),
        pem: database.hash(pubKey),
        meta: {
          created: now,
          updated: now
        },
...
api.getPublicKey = (publicKey, actor, callback) => {
  ...
  const publicKeyId = publicKey.id;
  let cacheKey;
 ...
 */
api.revokePublicKey = (actor, publicKeyId, callback) => {
  if(actor === undefined) {
    return callback(new BedrockError(
  ...
  async.auto({
    ...
    getPublicKey: ['cacheEvict', (results, callback) => api.getPublicKey(
...
api.checkKeyPairEd25519 = ({publicKey, privateKey = null}, callback) => {
  const {publicKeyBase58} = publicKey;

  // validate the public key material
  try {
    assert.string(publicKeyBase58);
    assert.equal(
  ...
  try {
    assert.string(privateKeyBase58);
    assert.equal(
  ...
  async.auto({
    sign: callback => signatureAlgorithm.sign(
      {algorithm: 'ed25519', plainText, privateKeyBase58}, callback),
    verify: ['sign', (results, callback) => signatureAlgorithm.verify(
      {plainText, publicKeyBase58, signature: results.sign}, callback)]
  }, (err, results) => {
